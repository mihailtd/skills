...
When implementing manual caching for specific data retrieval operations, you must strictly follow the Cache-Aside pattern.
*   **Check Cache First:** The application must first query Redis using a specific, deterministic cache key.
*   **Cache Hit:** If the data is present, deserialize it (e.g., from JSON) and return it immediately to the user without querying the primary database.
*   **Cache Miss:** If the data is absent, query the primary database (e.g., PostgreSQL, MongoDB, or Elasticsearch), store the serialized result in Redis, and then return the data.

## 2. Mandatory TTL (Time-To-Live)
Never cache API responses or dynamic database queries without an expiration time. 
*   When setting the value in Redis after a cache miss, you must use the `ex` parameter (e.g., `ex=3600`) to define the TTL in seconds. This prevents memory bloat and ensures stale data is eventually refreshed.

## 3. Declarative Caching with `fastapi-cache`
For standard endpoint caching where the entire response can be cached, prioritize using the `fastapi-cache` library rather than writing manual Redis queries.
*   The `fastapi-cache` library abstracts the cache-aside pattern into simple endpoint decorators.
*   Use the `@cache` decorator on your FastAPI route functions to easily define the TTL (`expire`), backend (Redis or in-memory), and encoding.
...
Use this approach for clean, readable endpoint caching.
