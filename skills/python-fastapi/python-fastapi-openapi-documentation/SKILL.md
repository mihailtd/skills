...
- Use `model_config = ConfigDict(json_schema_extra={...})` inside your Pydantic `BaseModel` definitions to embed example payloads.
- This ensures FastAPI correctly generates the JSON Schema payload examples in the interactive documentation.

## 2. Path Parameter Metadata

Do not leave URL path parameters undocumented.

- Import and use the `Path` class from the `fastapi` library to distinguish path parameters and add descriptive metadata.
- Use the `title` argument within the `Path` function to provide a clear description of what the parameter does (e.g., `todo_id: int = Path(..., title="The ID of the todo to retrieve")`).
...
- When instantiating an `APIRouter`, always assign a descriptive tag using the `tags` argument (e.g., `user_router = APIRouter(tags=["User"])`).
