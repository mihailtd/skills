...
*   Implement a `Result` class that encapsulates either a successful value or a standardized `Error` object.
...
*   **Application Layer:** Use cases must explicitly catch specific domain exceptions (like `ProjectNotFoundError`) and translate them into `Result.failure()` responses. 
