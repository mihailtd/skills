...
- **`groupby()`:** Use this to decompose a single iterable into a sequence of iterables over subsets of the input data. **Crucial Rule:** The input to `groupby()` _must_ be sorted by the matching key values prior to grouping, otherwise, it will not group identically keyed items correctly.
- **`chain()`:** Use this to combine multiple iterables serially into a single continuous sequence.
- **`zip_longest()`:** Use this when merging elements from multiple iterables of different lengths. Unlike the standard `zip()` function which truncates at the shortest list, `zip_longest()` pads the shorter iterables with a specified `fillvalue` (defaulting to `None`).

## 2. Filtering Sequences

Avoid materializing full lists just to extract subsets.

- **`compress()`:** Use this to filter one iterable based on a second, parallel iterable of Boolean values. This is highly efficient for applying a sequence of true/false rules to a data sequence.
- **`islice()`:** Use this to apply slice notation (start, stop, step) to an iterable or generator without the overhead of materializing the entire collection in memory.

## 3. Generating Combinatorics

When generating complex data arrangements, use the combinatorial functions but always consider computational complexity.

- **`product()`:** Use this to compute the Cartesian product of multiple collections. It acts as a highly optimized equivalent to nested `for` statements.
- **`combinations()`:** Use this to yield all possible subsets of an original set of length `r` in sorted order, without repeated elements. This is ideal when order does not matter.
- **`permutations()`:** Use this to yield tuples of length `r` in all possible orderings.
- **Combinatoric Warning:** You must proactively consider the size of the result set. Combinatorial explosion scales extremely poorly (e.g., $20!$ permutations would take roughly 12,000 years to compute). Never apply `permutations()` or `product()` to large collections without implementing a divide-and-conquer or filtering strategy.
