...
When writing a wrapper function inside a decorator, you must always apply the `@wraps(function)` decorator from the `functools` module to the wrapper.

- Python doesn't inherently adjust the internal bytecode of a function when decorating; it defines a new function that wraps the original.
- Using `@wraps` ensures that the resulting decorated function retains the metadata (like `__name__` and `__doc__` attributes) of the original base function.

## 2. Isolate Cross-Cutting Concerns

Do not clutter core application functions with infrastructure-level boilerplate.

- Use decorators to implement cross-cutting concerns like logging, auditing, security checks, and handling incomplete/bad data.
- Let the base function focus on the "happy path" (ideal processing), while the decorator intercepts arguments to sanitize data, or intercepts the return value to apply formatting.

## 3. Implement Null-Awareness Gracefully

In datasets where missing values (e.g., `None`) are common, avoid writing `if value is None:` inside every mathematical or domain function.

- Create a `@nullable` decorator that checks if the input is `None`.
- If it is `None`, immediately return `None`; otherwise, evaluate the base function.

## 4. Handle Bad Data and Parameterized Decorators

When dealing with strings that need cleaning before conversion (e.g., removing currency symbols or commas), use decorators to trap exceptions and retry the operation.

- Use a `try...except` block in the wrapper. If a `ValueError` occurs, clean the data and attempt the base function again.
- When your decorator needs configuration (like which characters to remove), create a parameterized decorator: an outer function that takes the arguments, returning an abstract decorator, which then wraps the base function.
